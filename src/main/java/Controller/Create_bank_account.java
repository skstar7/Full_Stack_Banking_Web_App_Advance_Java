...
public class Create_bank_account extends HttpServlet
{
	...
	Customer customer=(Customer) req.getSession().getAttribute("customer");
	      ...
	      List<Bank_account> list_bank_Account_type = customer.getBankaccounts();
	      
	      boolean flag=true;
	      
	     // System.out.println(account_Type);
	      
	      // this for each loop use for checking the account type already present with customer or not?
	      
	      for (Bank_account bank_account : list_bank_Account_type) 
	      {
...
//	    	 System.out.println(acc_type.equals(account_Type));
	      ...
	      if(flag==true)
	      {
	    	  ...
	    	  //list_of_Customer_bank_Account_type present inside a customer
	    	  //if savings already exist then curent account wiil be created for same customer
	    	  //if current already exist then savings account wiil be created for same customer
	    	  //if savings or current acccount not present for a customer then savings or current account will be created based radio button selected in account type ui
	    	  
	    	  List<Bank_account>  update_list_bank_Account_type=list_bank_Account_type;
	    	  ...
	    	  resp.getWriter().print("<h1>Congratulation your "+ acc_type+" bank account has created successfully"+"</h1>");
