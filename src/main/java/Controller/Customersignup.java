...
//				+ "<h1>"+dob+"<h1>");
		...
		Date date=Date.valueOf(dob);//here it is indicating that we ghave converted successfully String to datevalue-
...
//			resp.getWriter().print("Eligible");
			...
			resp.getWriter().print("<h1>Account has created Successfully your customerid is :"+customer1.getCid()+"<h1>");
			req.getRequestDispatcher("Customer_login.html").include(req, resp);
			}
			else 
			{
				resp.getWriter().print("<h1>This email id:"+eid+"and this mobile no:"+mob+"is already existed"+"<h1>");
			}
